...
class SGFLocator(object):
    ...
    def deserialize(cls, data):
        return SGFLocator(archive_path=data['archive_path'], archive_filename=data['archive_filename'])
...
def tarball_iterator(tarball_path):
    ...
    # Check for unsafe filenames. Theoretically a tarball can contain absolute filenames, or names like '../../whatever'
    ...
    sgf_names = [tf_entry.name for tf_entry in tf if tf_entry.isfile and tf_entry.name.endswith('.sgf')]
    sgf_names.sort()
    tf.extractall(tempdir)
    try:
        yield [SGF(SGFLocator(tarball_path, sgf_name), open(os.path.join(tempdir, sgf_name)).read()) for sgf_name in sgf_names]
    finally:
