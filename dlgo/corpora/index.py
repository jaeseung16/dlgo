...
def _sequence(game_record):
    ...
    The main sequence includes lots of stuff that is not actual game moves.
    """
    seq = []
    for item in game_record.get_main_sequence():
        color, move = item.get_move()
        # color == None is entries that are not actual game play
        # move == None is a pass, which in theory we could try to predict, but not yet
...
class Pointer(object):
    ...
    @classmethod
    def deserialize(cls, serialized):
        return cls(SGFLocator.deserialize(serialized['locator']), serialized['position'])
