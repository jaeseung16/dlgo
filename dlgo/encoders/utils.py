...
def is_ladder(try_capture, game_state, candidate, ladder_stones=None, recursion_depth=50):
    """
    Ladders are played out in reversed roles, one player tries to capture, the other to escape.
    We determine the ladder status by recursively calling is_ladder in  opposite roles, providing suiotable capture or escape candidates.
    ...
    candidate: a move that potentially leads to escaping the laddre or capturing it, instance of Move
    ...
    Returns True if  game state is a ladder and try_capture is true (the ladder captures) or if game state is not a ladder and try_capture is false (you can successfully escape) and False otherwise.
    """
...
def is_candidate(game_state, move, player):
    return game_state.next_player == player and count_liberties(game_state, move) == 2
