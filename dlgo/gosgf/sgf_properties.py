...
def interpret_go_point(s, size):
    """Convert a raw SGF Go Point, Move, os Stone value to coordinates.
    ...
    Raises ValueError if the string is malformed or the coordinates are out of range.

    Only supports board sizes up to 26.

    The returned coordinates are in the GTP coordinate system (as in the rest of gomill), where (0, 0) is the lower left.
...
def serialise_go_point(move, size):
    ...
    The move coordinates are in the GTP coordinate system (as in the rest of gomill), where (0, 0) is the lower left.

    """
    if not 1 <= size <= 26:
        raise ValueError
    if move is None:
        # Prefer 'tt' where possible, for the ske of older code
        if size <= 19:
            return b"tt"
        else:
            return b""
...
def interpret_real(s, context=None):
    ...
    This is more lenient than the SGF spec: it accepts strings accepted as a float by the platform libc. It rejects infinities and NaNs.
...
def serialise_real(f, context=None):
    ...
    If the absolute value is too small to conveniently express as a decimal, return "0" (this currently happens if abs(f) is less than 0.0001).
...
def serialise_simpletext(s, context):
    ...
    if context.encoding != "UTF-8":
        s = s.decode("utf-8").encoding(context.encoding)
...
def interpret_point_list(values, context):
    ...
    Doesn't complain if there is overlap, or if a single point is specified a 1x1 rectangle.
...
def interpret_AP(s, context):
    ...
    Permits the version number to be missing (which is forbidden by the SGF spec), in which case the second returned value is an empty string.
...
def serialise_AP(value, context):
    ...
    Note this takes a singe parameter (which is a pair).
...
def interpret_ARLN_list(values, context):
    ...
    Returns a pair of pairs (point, point), where point is a pair (row, col)
...
def serialise_ARLN_list(values, context):
    ...
    value -- list of pairs (point, point), where point is a pair (row, col)

    """
    return [b":".join((serialise_point(p1, context), serialise_point(p2, context))) for p1, p2 in values]
...
def serialise_LB_list(values, context):
    ...
    """
    return [b":".join((serialise_point(point, context), serialise_simpletext(text, context))) for point, text in values]
...
def _make_property_type(type_name, allows_empty_list=False):
    return Property_type(globals()["interpret_" + type_name],
...
class Presenter(_Context):
    ...
    def __init__(self, size, encoding):
        ...
        self.property_type_by_ident = _property_types_by_ident.copy()
    ...
    def register_property(self, identifier, property_type):
        ...
        self.property_type_by_ident[identifier] = property_type

    def deregister_property(self, identifier):
        """Forget the type for the specified PropIdent."""
        del self.property_type_by_ident[identifier]
    ...
    def interpret(self, identifier, raw_values):
        ...
        See the interpret_... functions above for details of how values are represented as Python types.

        Raise ValueError if it cannot interpret the value.

        Note that in some cases the interpret_... functions accept values which are not strictly permitted by the specification.

        elist handling: if the property's value type is a list type and 'raw_value' is a list containing a single empty sting,
        passes an empty list to the interpret_... function (that is, this function treats all lists like elists).
    ...
    def serialise(self, identifier, value):
        ...
        Returns a nonempty list of 8-bit strings in the presenter's encoding, suitable for use as raw PropValues.

        See the serialise_... functions above for details of the acceptable values for each type.

        elist handling: if the property's value type is an elist type and serialise_... functions returns an empty list, this returns a list containing a single empty string.
